...
const TEST_REGISTRY: PluginRegistry = {
  "@near-intents/axelar-data-provider": {
    remoteUrl: "http://localhost:3000/remoteEntry.js",
    version: "1.0.0",
    description: "Axelar data provider for integration testing",
  },
};

const TEST_PLUGIN_MAP = {
  "@near-intents/axelar-data-provider": DataProviderTemplatePlugin,
} as const;
...
describe("Data Provider Plugin Integration Tests", () => {
  ...
  beforeAll(async () => {
    const { initialized } = await runtime.usePlugin("@near-intents/axelar-data-provider", TEST_CONFIG);
    expect(initialized).toBeDefined();
    expect(initialized.plugin.id).toBe("@near-intents/axelar-data-provider");
  });

  describe("getSnapshot procedure", () => {
    it("should fetch complete snapshot successfully", async () => {
      const { client } = await runtime.usePlugin("@near-intents/axelar-data-provider", TEST_CONFIG);
